...
def run_dc_centerlines_pipeline(
    *,
    ...
    gdf = gpd.read_file(str(Path(input_path)), **read_kwargs)
    ...
    if streettype:
        if "STREETTYPE" not in gdf.columns:
    ...
    if gdf.empty:
        raise ValueError(
            "No rows remain after filtering. Note: 'Avenue' is typically STREETTYPE='AVE', not ROADTYPE."
        )
    ...
    centroids_ll = (
        gpd.GeoSeries(merged_gdf.geometry.centroid, crs=merged_gdf.crs).to_crs(epsg=4326)
        if merged_gdf.crs
        else merged_ll.geometry.centroid
    )
    ...
    names = [str(n) for n in merged_ll["original_name"].dropna().unique()]
    names_sorted = sorted(names)
    idx_map = {n: i for i, n in enumerate(names_sorted)}
    other = sorted(
        [n for n in names_sorted if n and not re.match(r"^(\\d+)", n) and not re.match(r"^([A-Z])\\s", n)]
    )
    ...
    _write_distances_csv(merged_ll[merged_ll["classification"] == "EW"].copy(), output_ew_csv_path)
    _write_distances_csv(merged_ll[merged_ll["classification"] == "NS"].copy(), output_ns_csv_path)
