...
from ..kml import kml_color_from_any, require_simplekml, write_kml_lines
...
def _kml_color_for_name(name: str, other_sorted: List[str], idx: int, n: int, spec: str) -> str:
    spec = str(spec or "dc-street-v1")
    if spec == "none":
        return "ff808080"
    ...
    if spec.startswith("cmap:"):
        ...
        return "ff" + hex_color[5:7] + hex_color[3:5] + hex_color[1:3]
    raise ValueError(
        f"Unknown style scheme {spec!r}. Use: dc-street-v1 | none | cmap:<name> | solid:<hex>."
    )
...
def run_dc_roadways_pipeline(
    *,
    ...
    gdf = gpd.read_file(str(Path(geojson_path)))
    if type_field not in gdf.columns or name_field not in gdf.columns:
        raise ValueError(f"Missing required columns: {type_field!r} and/or {name_field!r}")

    gdf = gdf[gdf[type_field] == road_type].copy()
    gdf_merge = gdf.to_crs(epsg=3857) if gdf.crs else gdf
    ...
    for name, group in gdf_merge.groupby(name_field):
    ...
    merged_gdf = gpd.GeoDataFrame(merged_rows, geometry="geometry", crs=gdf_merge.crs)
    if merged_gdf.empty:
        raise ValueError("No merged road geometries were produced.")

    merged_gdf["name"] = merged_gdf.apply(
        lambda row: f"{row['original_name']} Part-{row.get('part', 0)}", axis=1
    )

    merged_ll = merged_gdf.to_crs(epsg=4326) if merged_gdf.crs else merged_gdf.copy()
    if merged_gdf.crs:
        try:
            is_geographic = bool(getattr(merged_gdf.crs, "is_geographic", False))
        except Exception:
            is_geographic = False
        if is_geographic:
            centroids_proj = merged_gdf.to_crs(epsg=3857).geometry.centroid
            centroids_ll = gpd.GeoSeries(centroids_proj, crs="EPSG:3857").to_crs(epsg=4326)
        else:
            centroids_ll = gpd.GeoSeries(merged_gdf.geometry.centroid, crs=merged_gdf.crs).to_crs(epsg=4326)
    else:
        centroids_ll = merged_ll.geometry.centroid

    merged_ll["centroid"] = centroids_ll
    merged_ll["azimuth"] = merged_ll.geometry.apply(
        lambda line: _azimuth_deg_from_line([(float(c[0]), float(c[1])) for c in line.coords])
    )
    merged_ll["classification"] = merged_ll["azimuth"].apply(_classify_azimuth)
    merged_gdf = merged_ll
    ...
    names = [str(n) for n in merged_gdf["original_name"].dropna().unique()]
    names_sorted = sorted(names)
    idx_map = {n: i for i, n in enumerate(names_sorted)}
    other = sorted([n for n in names_sorted if n and not re.match(r"^(\\d+)", n) and not re.match(r"^([A-Z])\\s", n)])
    merged_gdf["kml_color"] = merged_gdf["original_name"].apply(
        lambda n: _kml_color_for_name(str(n), other, idx_map.get(str(n), 0), len(names_sorted), style_scheme)
    )
    ...
    _write_distances_csv(merged_gdf[merged_gdf["classification"] == "EW"].copy(), output_ew_csv_path)
    _write_distances_csv(merged_gdf[merged_gdf["classification"] == "NS"].copy(), output_ns_csv_path)
    _write_classified_kml(merged_gdf, output_kml_path, line_width=float(line_width))
...
def _write_distances_csv(gdf, output_path: Path) -> None:
    ...
    if gdf.empty:
        pd.DataFrame(
            columns=["StreetA", "LatA", "LonA", "StreetB", "LatB", "LonB", "Distance"]
