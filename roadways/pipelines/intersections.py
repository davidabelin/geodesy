...
def _dedupe_points(points: Iterable[IntersectionPoint], *, grid_size: float) -> List[IntersectionPoint]:
    if grid_size <= 0:
        return list(points)
    seen = set()
...
def run_intersections(
    *,
    ...
    a_kwargs = {}
    if a_layer:
        a_kwargs["layer"] = a_layer
    b_kwargs = {}
    if b_layer:
        b_kwargs["layer"] = b_layer

    a = gpd.read_file(str(a_input), **a_kwargs)
    b = gpd.read_file(str(b_input), **b_kwargs)
    ...
    joined = gpd.sjoin(
        b[[b_name_field_eff, "geometry"]].copy(),
        a[[a_name_field_eff, "geometry"]].copy(),
        how="inner",
    ...
    same_filters = (
        a_input.resolve() == b_input.resolve()
    ...
    for a_i, b_i in zip(a_idx.tolist(), b_idx.tolist()):
        if same_filters:
            ...
            if a_i > b_i:
                continue
        inter = a.geometry.iloc[a_i].intersection(b.geometry.iloc[b_i])
        for x, y in _extract_points(inter):
            points.append(
                IntersectionPoint(
                    a_name=str(a.iloc[a_i][a_name_field_eff]),
                    b_name=str(b.iloc[b_i][b_name_field_eff]),
                    x=float(x),
                    y=float(y),
                )
            )
