...
def get_top_ten_peaks_quadrant(dem_path, triad, buffer_m=100, separation_m=100):
    ...
    try:
        with rasterio.open(dem_path) as dataset:
            ...
            # Create a GeoDataFrame for the triangle in EPSG:4326.
            tri_gdf = gpd.GeoDataFrame(geometry=[triangle], crs="EPSG:4326")
...
def main():
    ...
    for triad in triads:
        ...
        peaks = get_top_ten_peaks_quadrant(dem_file, triad, buffer_m=buffer_m, separation_m=separation_m)
        if peaks:
            for peak in peaks:
