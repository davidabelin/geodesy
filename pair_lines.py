...
def process_line_pairs():
    ...
    """
    # Define file paths
    # Assuming the script is run from a location where these relative paths are valid,
    # or using absolute paths as provided in the context.
    base_data_folder = r"c:\Users\David\Documents\Local_Python\geodesy\data"
    
    trypoints_file = os.path.join(base_data_folder, "trypoints.csv")
    lppnts_file = os.path.join(base_data_folder, "LPPnts.csv")
    cappnts_file = os.path.join(base_data_folder, "CapPnts.csv")
    linepairs_file = os.path.join(base_data_folder, "linepairs.csv")
    output_file = os.path.join(base_data_folder, "paired_lines.csv")

    all_points_coords = {}

    # Load points with specified precedence:
    # CapPnts.csv first (lowest precedence)
    print(f"Loading data from {cappnts_file}...")
    load_coords_from_csv(cappnts_file, all_points_coords)
    # LPPnts.csv next (medium precedence)
    print(f"Loading data from {lppnts_file}...")
    load_coords_from_csv(lppnts_file, all_points_coords)
    # trypoints.csv last (highest precedence)
    print(f"Loading data from {trypoints_file}...")
    load_coords_from_csv(trypoints_file, all_points_coords)
